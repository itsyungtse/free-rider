...
/**
 * free-rider — drive Claude Code on this Mac from a Telegram chat.
 *
 * Flow:  Telegram app  ⇄  Telegram servers  ⇄  this bot (outbound long-poll)
 *                                                  └─▶ claude -p  (seat auth)
 *
 * Zero dependencies: uses Node's global fetch + child_process, and reads .env
 * via `node --env-file=.env` (see package.json "start").
 */
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (code) => {
      ...
      try {
        ...
        resolve({ ok: false, error: `could not parse claude output: ${e.message}\n${out.slice(0, 500)}` });
      }
    });
  });
}
...
async function handleUpdate(update) {
  ...
  if (text === '/whoami') {
    await sendMessage(chatId, `user id: ${fromId}\nwork dir: ${WORK_DIR}\npermission: ${PERMISSION_MODE}`);
    return;
  }
